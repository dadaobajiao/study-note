...
2.  1248 4812 byte short int long float double boolean（实际上是1/8字节） char 
...
5.  结论：String再也不用char\[\] 来存储了，改成了byte \[\] 加上编码标记，节约了一些空间
6.  同时基于String的数据结构，例如StringBuffer和StringBuilder也同样做了修改
...
1. 常量与常量的拼接结果在字符串常量池，原理是编译期优化
   ...
   ```ht
   再加载成字节码的时候就把字符串放进了字符串常量池中
   ```

   

1. 字符串常量池中不会存在相同内容的变量

2. 拼接前后，只要其中有一个是变量，结果就在堆中（就是字符串常量池之外，堆之内）。变量拼接的原理是StringBuilder
...
3. 如果拼接的结果调用intern()方法，根据该字符串是否在字符串常量池中存在，分为：
   *   如果存在，则返回字符串在常量池中的地址
...
![image-20220210143257756](C:\Users\打倒芭蕉\AppData\Roaming\Typora\typora-user-images\image-20220210143257756.png)
