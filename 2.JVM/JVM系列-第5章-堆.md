...
4.  所有的线程共享Java堆，在这里**还可以划分线程私有的缓冲区**（Thread Local Allocation Buffer，**TLAB**）。
...
1.  Java7 及之前堆内存**逻辑上**分为三部分：新生区+养老区+永久区
...
1. 在HotSpot中，Eden空间和另外两个survivor空间缺省所占的比例是**8 : 1 : 1，**
...
2、当我们进行一次垃圾收集后，红色的对象将会被回收，而绿色的独享还被占用着，存放在S0(Survivor From)区。同时我们给每个对象设置了一个**年龄计数器**，经过一次回收后还存在的对象，将其年龄加 1。
